...
一个**自举的语义技能发现系统**。首次运行自动建立领域知识图谱索引 + Sentence-Transformer Embedding 语义向量库，之后用中文自然语言查询就能精准找到技能。纯本地运行，秒级响应。
...
|---|---|---|
| 匹配方式 | 关键词 → npx 注册表 | 语义域 → 领域路由 + KG 扩展 + Embedding 精排 |
| 索引 | 无 | 本地知识图谱 + 向量库 |
| 查询延迟 | 秒级（需 npx 调用） | 2-3 秒（含 embedding 推理） |
| 失配率 | 高（"财报"搜不到"westock-data"） | 低（领域路由 + 语义向量覆盖泛化需求） |
| 跨领域组合 | 不支持 | 图谱邻接扩展 + 多意图自动检测 |
| 离线可用 | 否 | 是 |
| 语义理解 | 无 | ✅ 真正的语义相似度匹配
...
- [x] ✅ **Embedding 语义精排**：使用 `intfloat/multilingual-e5-small` 做真正的语义匹配
